...
class Admin{
	...
	public function new_admin($username,$password,$email,$fname,$lname,$cnumber){
		...
		$data = [
			[$username,$password,$email,$fname,$lname,$NOW,$NOW,'1', $cnumber],
		];
	...
	public function update_user($username,$email, $fname, $lname, $cnumber){
		...
		$q->execute(array(':fname'=>$fname, ':user_lastname'=>$lastname,':user_date_updated'=>$NOW,':user_time_updated'=>$NOW,':user_access'=>$access,':user_id'=>$id));
		return true;
	}
